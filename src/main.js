...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const ctx = { session:false, state:STATE.IDLE, idle:null, rec:null, recOn:false, pending:null, heygen:false };

  const idle = () => { clearTimeout(ctx.idle); ctx.idle = setTimeout(()=>say('Still there? (yes/no)', false), 20000); };

  const bgFromText = (t) => {
    if(/harvard/i.test(t)) return "/harvard-university-title.jpg";
    if(/oxford/i.test(t))  return "/oxford-university-title.jpg";
    if(/stanford/i.test(t))return "/stanford-university-title.jpg";
    return "/default-image.jpg";
  };
  const maybeBG = (t) => { els.container.style.background = `#000 url('${bgFromText(t)}') center/cover no-repeat`; };
  ...
  function initRec(){
    ...
    r.onerror = (ev)=>{ const sEl=document.getElementById('micStatus'); if(sEl) sEl.textContent='error'; console.warn('rec error', ev.error); ctx.recOn=false; els.mic.textContent='Mic: Off'; };
    return r;
  }
  ...
  console.log('Ready: Vite app initialized ✅');
});
