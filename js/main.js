document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Load KB
  let KB=[]; try{ const r=await fetch('./data/kb.json'); KB = await r.json(); }catch{}

  const STATE = { IDLE:'idle', CHOICE:'choice', ASK_NAME:'ask_name', ASK_VIDEO:'ask_video', PLAYING:'playing' };
  const ctx = { started:false, state:STATE.IDLE, name:null, university:null, rec:null, recOn:false };
  ...
  function bgFromUni(u){
    if(!u) return './public/default-image.jpg';
    if(/harvard/.test(u)) return './public/harvard-university-title.jpg';
    if(/oxford/.test(u))  return './public/oxford-university-title.jpg';
    if(/stanford/.test(u))return './public/stanford-university-title.jpg';
